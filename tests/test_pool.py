...
async def test_backend_manager_loads_via_rest_backend_device(device):
    """Pins the new shape: DeviceModelManager driven by an injected DeviceBackend
    (RestBackend.device), rather than owning management_url/headers/client itself."""
    backend = pool.RestBackend.device(device.url, poll_interval=0.01)
...
async def test_ensure_loaded_starts_when_absent(device):
    mgr = pool.DeviceModelManager(pool.RestBackend.device(device.url, poll_interval=0.01))
    await mgr.ensure_loaded("Qwen/Coder")
    assert ("start", "Qwen/Coder") in device.calls
    assert "Qwen/Coder" in device.running
    assert mgr.snapshot() == ["Qwen/Coder"]
...
async def test_unreachable_device_raises_device_error():
    mgr = pool.DeviceModelManager(pool.RestBackend.device(
