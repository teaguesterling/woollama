...
| **Capability-aware routing** — `<provider>/default` filters residents to those that can serve the endpoint, from the backend's own report where it publishes one and from `[inferencers.*.capabilities]` globs where it does not; pre-dispatch validation refuses a declared mismatch with a `400`; `GET /v1/models` annotates declared capability | `config.rs`, `pool.rs`, `lib.rs` | #20 |
| **Config variable contract** — `${VAR:-default}` (POSIX `:-`) in both implementations; a bare unset variable is a hard load error naming it, checked on the parsed structure so documentation is never mistaken for configuration; set-but-empty stays a deliberate choice, warned about only inside an `env` block | `expand_env`, `config.rs`, `config.py` | #21 |
