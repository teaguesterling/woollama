...
**Goal:** Bring the Python reference server (`src/woollama/`) to parity with the merged Rust `woollamad` for pluggable device management protocols — a `DeviceBackend` seam with built-in `device`/`ollama` + config-defined `rest` protocols — so the differential oracle is honest again.

**Architecture:** Mirror the Rust design (already shipped): extract `DeviceModelManager`'s hardcoded device HTTP (`pool.py::_running/_start/_stop`) into a `DeviceBackend` protocol; `RestBackend` (config-parameterized; `device` is a preset) + `OllamaBackend`; a `management_protocol` selector defaulting to `device`; `[management_protocols.<name>]` config parsing. The resolver/Gate/Slot/eviction-race logic is untouched.
...
- **Back-compat:** `management_url` with no `management_protocol` ⇒ `device`; the existing `tests/test_pool.py` (which uses a device-shaped `FakeDevice`) stays green — the `device` `RestBackend` reproduces `_running`/`_start`/`_stop` byte-for-byte.
...
### Task 1: `DeviceBackend` protocol + extract the `device` `RestBackend`; manager backend-driven
...
- Modify: `src/woollama/router.py` (lifespan `_pools` build — construct a `device` `RestBackend`)
...
Add `DeviceBackend`, `RestBackend` (+ `.device`), move `_running`/`_start`/`_stop`/`aclose` into `RestBackend` verbatim, and rewrite `DeviceModelManager.__init__`/`ensure_loaded`/`aclose` to use `self._backend`.

- [ ] **Step 4: Update existing pool tests + the router lifespan to the new ctor**

In `tests/test_pool.py`, replace every `DeviceModelManager(url, headers=…, poll_interval=…, …)` with `DeviceModelManager(RestBackend.device(url, headers=…, poll_interval=…, …), retry_after=…)`. In `src/woollama/router.py` lifespan (~line 145), `pool.DeviceModelManager(_inf.management_url, headers=_hdrs)` → `pool.DeviceModelManager(pool.RestBackend.device(_inf.management_url, headers=_hdrs))`.
...
git commit   # "refactor: DeviceBackend protocol + device RestBackend behind DeviceModelManager (py)" + trailers
```
...
- [ ] **Step 3: Implement `from_spec` + templating; re-express `device` via it**
...
**Spec/reference coverage:** `DeviceBackend` seam (T1) ✓; config `rest` (T3) ✓; `device` preset + back-compat default (T1+T3) ✓; `ollama` (T4) ✓; `management_protocol` field + parsing (T2) ✓; skip-bad-protocol + reserved-name warning (T3) ✓; case-insensitive header merge (T3, mirrors the Rust fix) ✓; resolver/Gate/eviction untouched (all tasks) ✓. Matches the merged Rust so the differential oracle is honest.
